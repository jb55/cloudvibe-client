...
class MyDialog(wx.Dialog):
  ...
  def onDropFiles(self, files):
    """Called when files are dropped on the main window"""
    songs = []
    for file in files:
      song = Song(file)
      song.load_all()
  ...
  def onListItemSelected(self, event):
    ...
    if song.artist:
      self.txtArtist.SetValue(song.artist)
    else:
      self.txtArtist.Clear()

    if song.title:
      self.txtTitle.SetValue(song.title)
    else:
      self.txtTitle.Clear()
  ...
  def onArtistChanged(self, event):
    if self.selectedListItem:
      ...
      self.onSongChanged(self.selectedSong)
